...
session_start();
$selqry="select * from tbl_purchase where customer_id='".$_GET["mid"]."' and purchase_status='0'";
...
if($result->num_rows>0)
{
	...
	if($row=$result->fetch_assoc())
	{
		...
		{
			 $selqry1="select * from tbl_purchase where customer_id='".$_GET["mid"]."' and purchase_status='0'";
			...
			{
			$insqry="insert into tbl_purchase(customer_id) value('".$_GET["mid"]."')";
			if ($con->query($insqry))
			{
				$selqry="select MAX(purchase_id) as id from tbl_purchase";
				$res=$con->query($selqry);
