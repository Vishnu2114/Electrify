...
<html>
    ...
    <?php
        if (isset($_POST["btn_checkout"])) {        
				...
				$selC = "select * from tbl_purchase where  customer_id='" .$_GET["cid"]. "'and purchase_status='0'";
                $rs = $con->query($selC);
                $row=$rs->fetch_assoc();
                $upQry1 = "update tbl_purchase set purchase_date=curdate(),purchase_amount='".$amt."',purchase_status='1' where  customer_id='" .$_GET["cid"]. "' and purchase_id=".$row["purchase_id"];
                if($con->query($upQry1))
                {
					...
					if($con->query($upQry1s))
					{
					 ...
					 $a = "update tbl_purchase set purchase_status='2' , purchase_time=curtime() where purchase_id='".$row["purchase_id"]."'";
				if($con->query($a))
				{
    ...
    <body onload="recalculateCart()">
        <input type="hidden" name="hid" id="hid" value="<?php echo $_GET['cid'] ?> "/>
        <h1>Cart</h1>
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_purchase b inner join tbl_cart c on c.purchase_id=b.purchase_id where b.customer_id='" .$_GET["cid"]. "' and purchase_status='0'";
               $res = $con->query($sel);
            ...
            <div class="product">
                <div class="product-image">
                    <img src="../Assets/Files/User/Photo/<?php echo $rowpr["product_photo"] ?>"/>
                </div>
        ...
        $(".product-quantity input").change(function() {
            ...
            $.ajax({
                url: "../Assets/AjaxPages/AjaxCustomerCart.php?action=Update&id=" + this.alt + "&qty=" + this.value +"&cid="+cid
            });
            updateQuantity(this);
        });
        $(".product-removal button").click(function() {
            $.ajax({
                url: "../Assets/AjaxPages/AjaxCustomerCart.php?action=Delete&id=" + this.value  +"&cid="+cid
            });
            removeItem(this);
        });
        /* Recalculate cart */
